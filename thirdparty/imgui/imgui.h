...
namespace ImGui
{
    ...
    IMGUI_API void          PushID(const std::string& str_id);                                     // push string into the ID stack (will hash string).
    ...
    // Widgets: Text
    IMGUI_API void          TextUnformatted(const std::string& text, const char* text_end = NULL);         // raw text without formatting. Roughly equivalent to Text("%s", text) but: A) doesn't require null terminated string if 'text_end' is specified, B) it's faster, no memory copy is done, no buffer size limits, recommended for long chunks of text.
    ...
    IMGUI_API bool          small_checkbox(const char* label, bool* v);
    ...
    IMGUI_API bool          BeginCombo(const std::string& label, const std::string& preview_value, ImGuiComboFlags flags = 0);
    ...
    IMGUI_API bool          InputTextA(const char* label, std::string* str, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
    IMGUI_API bool          InputText(const char* label, char* buf, size_t buf_size, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
    IMGUI_API bool          InputTextMultiline(const char* label, char* buf, size_t buf_size, const ImVec2& size = ImVec2(0, 0), ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
    IMGUI_API bool          InputTextWithHint(const char* label, const char* hint, char* buf, size_t buf_size, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
    IMGUI_API bool          InputTextWithHint(const std::string& label, const char* hint, std::string* buf, ImGuiInputTextFlags flags = 0, ImGuiInputTextCallback callback = NULL, void* user_data = NULL);
    ...
    IMGUI_API bool          Selectable(const std::string& label, bool* p_selected, ImGuiSelectableFlags flags = 0, const ImVec2& size = ImVec2(0, 0));       // "bool* p_selected" point to the selection state (read-write), as a convenient helper.

    // Widgets: List Boxes
    // - FIXME: To be consistent with all the newer API, ListBoxHeader/ListBoxFooter should in reality be called BeginListBox/EndListBox. Will rename them.
    IMGUI_API bool          ListBox(const char* label, int* current_item, const char* const items[], int items_count, int height_in_items = -1);
    IMGUI_API bool          ListBox(const char* label, int* current_item, bool(*items_getter)(void* data, int idx, const char** out_text), void* data, int items_count, int height_in_items = -1);
	IMGUI_API bool			ListBox(const std::string& label, const std::uint32_t* currIndex, std::vector<std::string>& values);
    ...
    IMGUI_API bool          BeginMenu(const std::string& label, bool enabled = true);                  // create a sub-menu entry. only call EndMenu() if this returns true!
    IMGUI_API void          EndMenu();                                                          // only call EndMenu() if BeginMenu() returns true!
    IMGUI_API bool          MenuItem(const std::string& label, const char* shortcut = NULL, bool selected = false, bool enabled = true);  // return true when activated. shortcuts are displayed for convenience but not processed by ImGui at the moment
    IMGUI_API bool          MenuItem(const std::string& label, const char* shortcut, bool* p_selected, bool enabled = true);              // return true when activated + toggle (*p_selected) if p_selected != NULL
    ...
    IMGUI_API void          LogToClipboardUnformatted(const std::string& fmt);                      // pass text data straight to log (without being displayed)
	...
	IMGUI_API bool          IsKeyPressedWithIndex(int user_key_index);
...
struct ImGuiTextFilter
{
	...
	IMGUI_API bool		Draw(const std::string& label, const std::string& hint, float width = 0.0f);
    IMGUI_API bool      PassFilter(std::string text, const char* text_end = NULL) const;
...
struct ImFontAtlas
{
    ...
    IMGUI_API std::string       GetSecondaryFonts();
    IMGUI_API std::string       GetPrimaryFont();
