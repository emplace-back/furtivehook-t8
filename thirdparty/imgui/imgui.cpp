...
#pragma warning (disable: 4996)             // 'This function or variable may be unsafe': ', strdup, sprintf, vsnprintf, sscanf, fopen
...
bool ImGuiTextFilter::Draw(const std::string& label, const std::string& hint, float width)
{
	if (width != 0.0f)
		ImGui::SetNextItemWidth(width);
	bool value_changed = ImGui::InputTextWithHint(label.data(), hint.data(), InputBuf, IM_ARRAYSIZE(InputBuf));
	if (value_changed)
		Build();
	return value_changed;
}
...
bool ImGuiTextFilter::PassFilter(std::string text, const char* text_end) const
{
    if (Filters.empty())
        return true;
    ...
    for (int i = 0; i != Filters.Size; i++)
    {
        ...
        {
            // Subtract
            if (ImStristr(text.data(), text_end, f.b + 1, f.e) != NULL)
                return false;
        }
        else
        {
            // Grep
            if (ImStristr(text.data(), text_end, f.b, f.e) != NULL)
                return true;
        }
    }
...
bool ImGui::IsKeyPressedWithIndex(int user_key_index)
{
	return ImGui::IsKeyPressed(ImGui::GetKeyIndex(user_key_index));
}
...
{
    ImGuiWindow* window = GImGui->CurrentWindow;
    window->IDStack.push_back(window->GetIDNoKeepAlive(str_id.data()));
}
...
bool ImGui::BeginPopup(const char* str_id, ImGuiWindowFlags flags)
{
    ...
    flags |= ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoSavedSettings | ImGuiWindowFlags_NoMove;
...
const ImGuiPayload* ImGui::AcceptDragDropPayload(const char* type, ImGuiDragDropFlags flags)
{
    ...
    payload.Delivery = was_accepted_previously && !IsMouseDown(g.DragDropMouseButton); // For  drag sources affecting os window focus, it's easier to just test !IsMouseDown() instead of IsMouseReleased()
...
void ImGui::LogToClipboardUnformatted(const std::string& fmt)
{
    ImGui::LogToClipboard();
    ImGui::LogText(fmt.data());
    ImGui::LogFinish();
}
